...
# 부모 디렉토리(프로젝트 루트)에 있는 .env 파일 로드
load_dotenv(os.path.join(os.path.dirname(BASE_DIR), ".env"))

app = Flask(__name__)
app.secret_key = os.environ.get("AUTH_SECRET_KEY", "fallback-secret-key")
...
def init_db():
    with app.app_context():
        ...
        if not cur.fetchone():
            initial_pw = os.environ.get("INITIAL_ADMIN_PASSWORD", "admin1234")
